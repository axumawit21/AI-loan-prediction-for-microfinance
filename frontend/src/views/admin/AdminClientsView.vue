<template>
  <div class="space-y-6 animate-fade-in">
    <div>
      <h1 class="text-xl font-bold text-slate-900">Client Directory</h1>
      <p class="text-sm text-slate-500 mt-0.5">All registered SACCO members (read-only)</p>
    </div>
    ...
    <div class="grid grid-cols-3 gap-4">
      <div class="bg-white rounded-xl border border-slate-200/60 p-4 text-center">
        <p class="text-2xl font-bold text-slate-900">{{ clients.length }}</p>
        <p class="text-xs text-slate-500 mt-1">Total Clients</p>
      </div>
      <div class="bg-white rounded-xl border border-slate-200/60 p-4 text-center">
        <p class="text-2xl font-bold text-emerald-600">{{ clients.filter(c => c.status === 'active').length }}</p>
        <p class="text-xs text-slate-500 mt-1">Active</p>
      </div>
      <div class="bg-white rounded-xl border border-slate-200/60 p-4 text-center">
        <p class="text-2xl font-bold text-blue-600">{{ totalSavings }}</p>
        <p class="text-xs text-slate-500 mt-1">Total Savings (ETB)</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200/60 overflow-hidden">
      <table class="w-full" v-if="clients.length > 0">
        ...
        <tbody>
          <tr v-for="c in clients" :key="c._id" class="hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold" :style="{ background: avatarColor(c.name) }">{{ initials(c.name) }}</div>
                <div>
            ...
            <td class="px-4 py-4">
              <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold" :class="c.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'">{{ c.status }}</span>
            </td>
          </tr>
        </tbody>
      </table>
...
const totalSavings = computed(() => clients.value.reduce((sum, c) => sum + (c.totalSavings || 0), 0).toLocaleString())

async function loadClients() {
  const res = await fetchClients({ search: search.value, limit: 100 })
