<template>
  <div class="space-y-6 animate-fade-in">
    <div>
      <h1 class="text-xl font-bold text-slate-900">All Loans</h1>
      <p class="text-sm text-slate-500 mt-0.5">System-wide loan applications browser</p>
    </div>
    ...
    <div class="flex flex-wrap gap-4">
      ...
      <select v-model="statusFilter" @change="loadLoans" class="px-4 py-2.5 border border-slate-200 rounded-lg text-sm text-slate-800 bg-white min-w-[150px]">
      ...
      <select v-model="riskFilter" @change="loadLoans" class="px-4 py-2.5 border border-slate-200 rounded-lg text-sm text-slate-800 bg-white min-w-[140px]">
    ...
    <div class="bg-white rounded-xl border border-slate-200/60 overflow-hidden">
      <table class="w-full" v-if="loans.length > 0">
        ...
        <tbody>
          <tr v-for="loan in loans" :key="loan._id" class="hover:bg-slate-50 transition-colors cursor-pointer border-b border-slate-50 last:border-0" @click="$router.push('/admin/loans/' + loan._id)">
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold" :style="{ background: avatarColor(loan.clientId?.name) }">{{ initials(loan.clientId?.name) }}</div>
                <div>
            ...
            <td class="px-4 py-4">
              <span v-if="loan.prediction" :class="riskClass(loan.prediction.riskLevel)" class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-bold uppercase">{{ loan.prediction.riskLevel }} {{ loan.prediction.riskScore }}</span>
            ...
            <td class="px-4 py-4">
              <button @click.stop="$router.push('/admin/loans/' + loan._id)" class="px-4 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-md transition-colors">View</button>
            </td>
          </tr>
        </tbody>
      </table>
      <div v-else class="p-12 text-center text-slate-400 text-sm">No loans found.</div>
    </div>
  </div>
</template>
...
async function loadLoans() {
  const res = await fetchLoans({ search: search.value, status: statusFilter.value, riskLevel: riskFilter.value, limit: 50 })
